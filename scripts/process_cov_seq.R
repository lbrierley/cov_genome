...
n_man_genbank_seqs <- sarscov2_GenBank_FASTA[lapply(sarscov2_GenBank_FASTA, function(x) {
  x %>%
    attributes() %>%
    .$Annot %>%
    grepl("gene=S", .)
}) %>% unlist()] %>% length() # total S sequences - should be larger but it's smaller?? some might be missing annotation?
...
allcov_df <- merge(allcov_df,
                   cov_wg_df %>%
                     group_by(taxid) %>%
                     summarise(
                       mean_wgs_length = mean(CDS_length, na.rm = TRUE),
...
n_cov_seq <- allcov_df %>%
  filter(n_seqs > 0) %>%
  nrow() # n coronaviruses with at least 1 sequence
n_cov_seq_eid2 <- allcov_df %>%
  filter(n_seqs > 0 & eid2_n_species > 0) %>%
  nrow() # n coronaviruses with at least 1 sequence and at least 1 EID2 host
...
all.x = TRUE
  ...
  select(accessionversion, title, taxid, genus, childtaxa_name, createdate, complete, length, subname) %>%
  write.csv(., "data\\allcov_spikes_metadata.csv")
