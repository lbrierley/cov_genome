...
**S1, S2:**  *only `r allcov_df %>% filter(n_S1 > 0) %>% nrow` and `r allcov_df %>% filter(n_S2 > 0) %>% nrow` have complete sequences for isolated subunits, respectively. (S1 = `r allcov_df %>% filter(n_S1 > 0) %>% .$childtaxa_name`, S2 = `r allcov_df %>% filter(n_S2 > 0) %>% .$childtaxa_name`)*
